...
class YelpDataset(Dataset):
    def __init__(self, data, path_num, timestamps, adj_BCa, adj_BCi, adj_UB, adj_UU, negatives, sample, mode):
        ...
        for u in range(self.n_user):
            ...
            self.user_uu[u] = set(uu)
            self.user_ub[u] = set(ub)
        ...
        if self.mode == 'train':
            ...
            for i in range(n_path):
                ...
                for item in items:
                    ...
                    path_input.append(paths)
                path_inputs.append(torch.tensor(path_input, dtype = torch.int64))
            return np.array([user] * (self.n_negative + 1)), np.array(items), np.array([1.0] + [0.0] * self.n_negative, dtype=np.float32), path_inputs
        else:
            user = self.data[index]['user_id']
            ...
            for i in range(n_path):
                ...
                for item in items:
                    ...
                    path_input.append(paths)
                path_inputs.append(torch.tensor(path_input, dtype = torch.int64))
                # print(torch.tensor(path_input, dtype = torch.int64).shape)
            return np.array([user] * (pos_n + neg_n)), np.array(items), np.array([1.0] * pos_n + [0.0] * neg_n, dtype=np.float32), path_inputs
