...
class MCRec(nn.Module):
    def __init__(self, n_type, path_nums, timestamps, latent_dim, path_type, device):
        ...
        self.path_embedding = [Path_Embedding(latent_dim, latent_dim, n_type, device).to(device) for i in range(len(path_nums))]
        ...
        for i in range(2):
            ...
            mlp.append(nn.Linear(input_size, input_size // 2).to(device))
            mlp.append(nn.ReLU().to(device))
        self.mlp = nn.Sequential(*mlp)
        self.prediction_layer = nn.Linear(latent_dim * 3 // 4, 1).to(device)
