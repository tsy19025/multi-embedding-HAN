...
if __name__ == '__main__':
    ...
    if args.dataset == 'yelp':
        ...
        train_data_loader = DataLoader(dataset = YelpDataset(users, items, train_data, paths, path_num, timestamps, adj_UB, args.negetives, 'train'),
        ...
        valid_data_loader = DataLoader(dataset = YelpDataset(users, items, valid_data, paths, path_num, timestamps, adj_UB, 0, 'valid'),
                                       batch_size = 1,
                                       shuffle = True,
                                       num_workers = 20,
                                       pin_memory = True)
        ...
        test_data_loader = DataLoader(dataset = YelpDataset(users, items, valid_data, paths, path_num, timestamps, adj_UB, 0, 'test'),
                                      batch_size = 1, 
                                      shuffle = True,
                                      num_workers = 20,
                                      pin_memory = True)
    ...
    for epoch in range(args.epochs):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(state, '/home1/tsy/multi-embedding-HAN/tmp/model/modelpara.pth')
            print('Model save for lower valid loss %f' % best_loss)

    # test
    model.load_state_dict(torch.load('/home1/tsy/multi-embedding-HAN/tmp/model/modelpara.pth'))
    model.to(device)
